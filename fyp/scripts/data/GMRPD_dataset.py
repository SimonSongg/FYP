...
class GMRPDdataset (BaseDataset):
    ...
    def initialize(self, opt):
        ...
        if opt.phase == "train":
            self.image_list = sorted(glob.glob(os.path.join(self.root, 'train', 'rgb', '*.png')))
        elif opt.phase == "val":
            self.image_list = sorted(glob.glob(os.path.join(self.root, 'val', 'rgb', '*.png')))
        else:
            self.image_list = sorted(glob.glob(os.path.join(self.root, 'test', 'rgb', '*.png')))
    ...
    def __getitem__(self, index):
        ...
        if self.opt.phase == 'test' and self.opt.no_label:
            ...
            label_image = cv2.cvtColor(cv2.imread(os.path.join(useDir, 'gt', name)), cv2.COLOR_BGR2RGB)
            label = np.zeros((oriHeight, oriWidth), dtype=np.uint8)
            label[label_image[:,:,2] == 1] = 1
        ...
        if self.use_sne:
            camParam = torch.tensor([[1361.400024, 0, 955.198975],
                                     [0, 1361.399902, 511.039001],
